...
const midTopRow = ["D", "C", "B", "A"];
const midBottomRow = ["D", "B", "A", "C"];
...
function print(pos: Position): void {
  ...
  console.log("###" + pos.rooms.map((r) => r[3] ?? " ").join("#") + "###");
  console.log("  #" + pos.rooms.map((r) => r[2] ?? " ").join("#") + "#  ");
...
function generateNextPositions(pos: Position): [number, Position][] {
  ...
  pos.hallway.forEach((h, i) => {
    if (
      !h ||
      pos.rooms[targetRooms[h]].length === ROOM_SIZE ||
    ...
    const score =
      (Math.abs(start - end) + (ROOM_SIZE - pos.rooms[targetRooms[h]].length)) *
      moveFactors[h];
  ...
  pos.rooms.forEach((r, i) => {
    ...
    const baseScore = moveFactors[toMove] * (ROOM_SIZE - r.length + 1);
...
function isSolved(pos: Position): boolean {
  return ["A", "B", "C", "D"].every(
    (letter, roomIndex) =>
      pos.rooms[roomIndex].length === ROOM_SIZE &&
