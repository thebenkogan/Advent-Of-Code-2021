...
const applyOnce = applyAlgo(image, 0);
const applyTwice = applyAlgo(applyOnce, SHOULD_ALTERNATE_FILLS ? 1 : 0);

let lit = 0;
for (let i = 0; i < applyTwice.length; i++) {
  for (let j = 0; j < applyTwice[0].length; j++) {
    lit += applyTwice[i][j];
  }
}
